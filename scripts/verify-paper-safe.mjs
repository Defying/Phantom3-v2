...
await expectFile('scripts/verify-trading-preference.ts', 'trading preference verifier exists');
...
await expectIncludes('package.json', '"verify:trading-preference"', 'package.json exposes the trading preference verifier');
...
await expectIncludes('README.md', 'verify:trading-preference', 'README documents the trading preference verifier');
...
await expectIncludes('apps/api/src/app.ts', 'legacy profiles stay reference-only', 'access note stays honest about legacy parity posture');
await expectIncludes('packages/strategy/src/profiles.ts', 'Reference-only for now.', 'legacy trading-preference options stay reference-only');
...
await expectIncludes('docs/qa/PAPER_SAFE_STRATEGY_CHECKLIST.md', 'verify:trading-preference', 'QA checklist includes the trading preference verifier');
...
await expectIncludes('docs/runbooks/PAPER_SAFE_OPERATOR_RUNBOOK.md', 'verify:trading-preference', 'operator runbook includes the trading preference verifier');
...
console.log('Note: this script only validates repository guardrails and documentation markers. Pair it with npm run verify:paper-runtime for ledger truth and sanitized paper API checks, plus npm run verify:trading-preference for control-plane, persistence, and legacy-reference parity checks.');
